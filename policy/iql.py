...
class IQL:
    def __init__(self, args, master):
        ...
        stamp = int(time.time())
        if not self.args.dr_VK1:
            self.model_dir = self.args.model_dir + '/' + args.alg + '/no_dr/' + '%f'%args.level  + '/' + (time.strftime("%Y_%m_%d_%H", time.localtime(stamp)))
        else:
            self.model_dir = self.args.model_dir + '/' + args.alg + '/dr/' + '%f'%args.level  + '/' + (time.strftime("%Y_%m_%d_%H", time.localtime(stamp)))
        # # 如果存在模型则加载模型
    ...
    def learn(self, batch, train_step, target_update):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        s, s_next, u, r, avail_u, avail_u_next, terminated = batch['s'], batch['s_next'], batch['u'], \
                                                             batch['r'].repeat(1, self.n_agents_per_party),  batch['avail_u'], batch['avail_u_next'],\
                                                             batch['terminated'].repeat(1, self.n_agents_per_party)


        mask = 1 - batch["padded"].float().repeat(1, self.n_agents_per_party)  # 用来把那些填充的经验的TD-error置0，从而不让它们影响到学习

        # 得到每个agent对应的Q值，维度为(transition_num， n_agents， n_actions)
    ...
    def save_model(self, eps):
        ...
        # torch.save(self.eval_qmix_net.state_dict(), self.model_dir + '/' + str(eps) + '_qmix_net_params.pkl')
