import torch
import os
from network.qmix_net import QMixNet, myRNN
import time 
...
class QMIX:
    def __init__(self, args, master):
        ...
        stamp = int(time.time())
        self.model_dir = args.model_dir + '/' + args.alg + '/' + '%f'%args.level + '/' + (time.strftime("%Y_%m_%d_%H", time.localtime(stamp)))
        # 如果存在模型则加载模型
        '''
    ...
    def load_model(self, eps):
        if os.path.exists(self.model_dir + '/' + str(eps) + '_rnn_net_params.pkl'):
            ...
            path_qmix = self.model_dir + '/' + str(eps) + '_qmix_net_params.pkl'
            map_location = 'cuda:0' if self.args.cuda else 'cpu'
            self.eval_rnn.load_state_dict(torch.load(path_rnn, map_location=map_location))
            self.eval_qmix_net.load_state_dict(torch.load(path_qmix, map_location=map_location))
            print('Successfully load the model: {} and {}'.format(path_rnn, path_qmix))
